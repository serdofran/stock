			  				...
			  				<td style="margin-left:20px;">
			  					...
			  					<select class="form-control" name="productName[]" id="productName<?php echo $x; ?>" onchange="getProductData(<?php echo $x; ?>)" >
			  						...
			  						/*	$productSql = "SELECT * FROM product WHERE active = 1 AND status = 1 AND quantity != 0";
										  ...
										  $stm =$pdo->prepare("SELECT * FROM product WHERE active=? AND status=? AND quantity !=?");
  		...
  		<form class="form-horizontal" method="POST" action="php_action/editOrder.php" id="editOrderForm">
			  ...
			  <table class="table" id="productTable">
			  	...
			  	/*	$orderItemSql = "SELECT order_item.order_item_id, order_item.order_id, order_item.product_id, order_item.quantity, order_item.rate, order_item.total FROM order_item WHERE order_item.order_id = {$orderId}";
						...
						$stm =$pdo->prepare("SELECT order_item.order_item_id, order_item.order_id, order_item.product_id, order_item.quantity, order_item.rate, order_item.total FROM order_item WHERE order_item.order_id = ?");
